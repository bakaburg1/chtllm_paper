...
test_that("extract_answer extracts correctly", {
    ...
    expect_match(extract_answer(response), "C")
    ...
    expect_equal(extract_answer("No answer here"), "NONE*")
    expect_equal(extract_answer("_E_"), "NONE*")
})
...
test_that("query_llm returns expected structure", {
    ...
    message <- get_formatted_question(questions, 1)
    ...
    # Run query
    result <- query_llm(message, model_config, "cold")
    ...
    expect_match(extract_answer(result$answer), "^[ABCD]\\*?$")
    ...
    expect_s3_class(result$metadata$generation_time, "difftime")
    ...
    expect_equal(result$config$model_id, model_config$model_id)
    expect_equal(result$config$provider, model_config$provider)
    expect_equal(result$config$model_name, model_config$model_name)
    expect_equal(result$config$modality, "cold")
    expect_equal(result$config$temperature, 0)

    # Test inputs
    expect_type(result$inputs, "list")
    expect_named(result$inputs, c("modality", "message"))
    expect_equal(result$inputs$modality, "cold")
...
test_that("query_llm works with different modalities", {
    ...
    for (modality in modalities) {
        result <- query_llm(message, model_config, modality)

        # Basic structure checks
        expect_type(result, "list")
        expect_named(result, c("answer", "metadata", "config", "inputs"))

        # Check modality is correctly set
        expect_equal(result$config$modality, modality)
        expect_equal(result$inputs$modality, modality)
...
test_that("query_llm handles model_id input", {
    ...
    result <- query_llm(
        message, "qwen2.5-coder-1.5b-instruct-mlx@8bit", "cold")
