...
#| fig-cap: "Effect of prompt strategy on model accuracy. Points show the posterior median and bars the 95% credible intervals."
#| fig-width: 7
#| fig-height: 6
...
Detailed median posterior performance metrics for each model are reported in Supplementary Table S3.\
...
Prompting altered these figures only marginally (@fig-modality-correctness); detailed prompt-by-model-type estimates are shown in Supplementary Table S4 and S5. Classic models improved from {acc_classic_cold$prob} with a **cold** prompt to {acc_classic_reason$prob} under explicit reasoning instructions, whereas reasoning models performed best with the concise **cold** prompt ({acc_reasoning_cold$prob} vs {acc_reasoning_free$prob} for a *free* prompt). But the largest performance gap was between reasoning models and classics, independent of prompt strategy.
```
...
p_parsing <- parsing_plots$by_model + labs(title = "A. Parsing success")
p_consistency <- consistency_plots$by_model + labs(title = "B. Answer consistency")
...
Most models followed formatting instructions and produced the expected output format, which allowed for automated parsing of the answers ([@fig-metrics]A). Parsability exceeded 90% for {num_high_parse} of the {num_models} models we evaluated. The distribution was decidedly bimodal: at one extreme, large high-performing frontier models such as `o3` ({parse_o3$prob}), `gemini-2.5-pro` ({parse_gempro$prob}), and `deepseek-r1` ({parse_deepseek$prob}) yielded rates well above 95%; at the other, small locally-running models showed a strong drop in parsability, achieving barely half that rate with scores around 50%.\
Supplementary Table S3 reports the full set of posterior estimates.
...
Supplementary Table S3 tabulates the posterior consistency estimates for each model.
...
Very few models were in the top-left quadrant (mostly smaller versions of high-performing models, such as o4-mini and Claude 3.5 "Haiku", but also Llama 4 "Scout") or the bottom-right quadrant (accurate but inconsistent models, notably two search-enabled models, Perplexity Sonar Large and GPT-4o-search).
```
...
item_difficulty <- summaries_correctness_by_model_item |>
  dplyr::summarise(med = median(.prob), .by = "item") |>
  dplyr::arrange(med)
...
correct_prop <- summaries_correctness_by_model_item |>
  dplyr::filter(.data$item == item_hard_id) |>
  dplyr::summarise(prop = mean(.data$.prob > 0.5)) |>
  dplyr::pull(prop) |>
...
ans_o3 <- results_raw |>
  dplyr::filter(.data$item == item_hard_id, .data$model_id == "o3") |>
  dplyr::mutate(answer_clean = stringr::str_extract(answer, "[A-D]")) |>
  dplyr::count(answer_clean, name = "n") |>
  dplyr::mutate(pct = scales::percent(n / sum(n), 1))
