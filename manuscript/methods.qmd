...
All models were fitted in Stan [@stan] through the *brms* [@brms] interface and included a random intercept for items, a random intercept by model, as well as a random slope for the interaction between prompt strategy and model.
...
\begin{aligned}
\pi_{n0} &= \text{logit}^{-1}(\kappa_0 - \eta_n), \\
\pi_{n1} &= \text{logit}^{-1}(\kappa_1 - \eta_n) - \text{logit}^{-1}(\kappa_0 - \eta_n), \\
\pi_{n2} &= 1 - \text{logit}^{-1}(\kappa_1 - \eta_n).
\end{aligned}
